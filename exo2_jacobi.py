...
PHInew = np.zeros([Ny,Nx], dtype='float64') # Store new PHI values
    ...
    for j in range(0,Nx):
        for i in range(0,Ny):          
            ...
            if r_current > rBC:
                ...
                PHInew[i,j] = PHI[i,j]+theta*(deltaR[i,j]/(-2/dy**2-2/dx**2))
...
print('Runime: ', round(simTimeEnd,0), 'sec')
...
ax2 = axes[0,1]
c2  = ax2.pcolor(x,y,PHI,cmap='RdBu')
ax2.set_title('colormap of PHI')
fig.colorbar(c2, ax=ax2)

ax3 = axes[1,0]
c3  = ax3.pcolor(x,y,gX,cmap='RdBu')
ax3.set_title('colormap of gX')
fig.colorbar(c3, ax=ax3)

ax4 = axes[1,1]
c4  = ax4.pcolor(x,y,gY,cmap='RdBu')
